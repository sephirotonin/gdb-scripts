...
DUMP_DIR="/home/user/Schreibtisch/fuzzing/HIER1"
mkdir -p "$DUMP_DIR"
...
commands
    ...
    set \$input_size = \$x2  
    printf "Input size: 0x%lx bytes\\n", \$input_size
    eval "dump binary memory $DUMP_DIR/input_dump_${TIMESTAMP}_y%d.bin \$x1 \$x1 + \$input_size", \$counter
    continue
end
continue
EOF
