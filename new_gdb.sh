...
sleep 5
DUMP_DIR="/home/user/Schreibtisch/fuzzing/HIER5"
mkdir -p "$DUMP_DIR"
...
commands
    ...
    set \$cmd = \$x0 
    set \$input_size = \$x2
    printf "Hit bp_keymaster_call (%d) at %p, cmd=0x%lx, input_buf=%p, input_size=0x%lx\\n", \$counter, \$pc, \$cmd, \$x1, \$input_size
    eval "dump binary memory $DUMP_DIR/input_dump_${TIMESTAMP}_cmd_0x%lx_y%d.bin \$x1 \$x1 + \$input_size", \$cmd, \$counter
    continue
end
continue
EOF
